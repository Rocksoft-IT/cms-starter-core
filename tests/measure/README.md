...
OLD_BASE_URL=https://www.example.com pnpm test:measure   # a deployed reference
OLD_BASE_URL=../design-export pnpm test:measure          # …or the design repo itself
```

**`OLD_BASE_URL` takes a path as well as a URL**, and the path is the one to reach for first. A
design is handed over as a static HTML/CSS/JS repo, so on day one of a port there is nothing
deployed to compare against — which used to mean this harness could not be used until somebody put
the design on a server, and that the first pass was done by eye instead. Point it at the folder and
the run serves it itself, on a loopback port it owns for the length of the run
(`../shared/static-reference.js`). Offline, deterministic, and usable in CI.

It also resolves `/uslugi` to the export's `uslugi.html`, so the same address works on both sides
and `old_path` is needed only for a genuine rename — not, as before, to bridge the gap between a
folder of `.html` files and a site of directories.

Two things to know before trusting the numbers:

- **The reference has to be the right one.** This is the failure the field exists to make visible:
  kwalitet's baseline was recorded from the client's live domain, which still served the design the
  port REPLACES, and every target reported agreement for weeks (kwalitet-pl#2). Serving the design
  repo removes the question — the artefact the designer handed over cannot be the wrong version of
  itself. Each baseline records what it measured under `reference`.
- **Webfonts have to resolve.** A design export loads its faces from a CDN. Served locally with no
  network they fail, the text is laid out in a fallback, and every text width is wrong while every
  box stays right. On a dev machine this is automatic; in CI, install the faces (the export's
  `WebFont.load`/`@font-face` names them) or the text metrics mean nothing.
...
| env                                | effect                                                                                                                                                 |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `OLD_BASE_URL`                     | the reference: an origin (`https://…`) **or a path to the static design repo**, which the run serves itself — **required** unless `MEASURE_BASELINE=1` |
