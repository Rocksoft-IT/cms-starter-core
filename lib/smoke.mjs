...
//   • consent/tag      — a page loading a Google tag also ships the consent banner that tag is
//                        gated behind [REPORT; catches a half-mounted consent setup and any
//                        hand-rolled tag added outside the CMS chain — dashboard #1191];
...
//                        this offline check].
//
// Redirect stubs are skipped, not checked — see isRedirectStub().
//
...
/**
 * Is this file a build-emitted redirect stub rather than a page?
 *
 * Astro writes one per `redirects` entry on a static build — the map core's cmsRedirects() pulls
 * from the CMS (dashboard #1084). It is deliberately minimal: a meta refresh, `noindex` and a
 * canonical at the destination, with no `<html>` wrapper, no description and no share image. Every
 * check below would fail it, and each of those failures would be asking a redirect to be a page.
 * Detected by the refresh itself, not by a path pattern: the file lives at whatever URL was
 * vacated, which is indistinguishable from a real route by name.
 */
function isRedirectStub(scope) {
  for (const [tag] of scope.matchAll(/<meta\b[^>]*>/gi)) {
    if (attr(tag, 'http-equiv')?.toLowerCase() === 'refresh') return true
  }
  return false
}
...
const warnings = [] // REPORT-only (og:image by default; consent/tag consistency always)
let stubs = 0 // redirect stubs skipped — reported, never silently dropped
...
for (const file of files) {
  ...
  // consent consistency — a page that loads a Google tag must also carry the banner that tag is
  // supposed to be gated behind (REPORT). Two real states produce this: a site that mounts
  // <ConsentMode/> but forgot <CookieConsent/>, and a hand-rolled tag pasted into a layout
  // outside the CMS chain entirely (dashboard #1191). Non-fatal on purpose — this ships to every
  // client through node_modules, and a pre-existing hardcoded tag must not break their deploy.
  // `(?![-\w])` matches the banner's own bare attribute and NOT `data-cookie-consent-open`, the
  // reopen hook — a site can carry an opener link (or, as the starter does, a comment mentioning
  // one) on a page that ships no banner at all.
  if (/googletagmanager\.com/i.test(html) && !/data-cookie-consent(?![-\w])/i.test(html)) {
    warnings.push(`${rel}: loads a Google tag but ships no cookie-consent banner`)
  }
...
if (warnings.length > 0) {
  console.warn(`[smoke] ${warnings.length} warning(s) (not fatal; --strict-og enforces og:image):`)
  for (const w of warnings) console.warn(`  ⚠ ${w}`)
}

const pages = files.length - stubs
const skipped = stubs ? ` (${stubs} redirect stub(s) skipped)` : ''

if (errors.length > 0) {
  console.error(`[smoke] ${errors.length} violation(s) across ${pages} page(s)${skipped}:`)
  for (const e of errors) console.error(`  ✗ ${e}`)
  process.exit(1)
}

console.log(
  `[smoke] OK — ${pages} page(s): title + meta description on every page` +
    (strictOg ? ' + og:image' : '') +
    `, forms have submit targets.${skipped}` +
